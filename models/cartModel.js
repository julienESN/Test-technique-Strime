...
exports.removeProduct = async (cartId, productId) => {
  return db.none('DELETE FROM "CartItems" WHERE cart_id = $1 AND product_id = $2', [
    cartId,
    productId,
  ]);
};
...
exports.checkout = async (cartId) => {
  return db.tx(async (t) => {
    const items = await t.any('SELECT * FROM "CartItems" WHERE cart_id = $1', [cartId]);
    for (let item of items) {
      await t.none(
        'INSERT INTO "Order" (user_id, product_id, quantity) VALUES ($1, $2, $3)',
        [item.user_id, item.product_id, item.quantity]
      );
      await t.none('DELETE FROM "CartItems" WHERE cart_id = $1', [item.cart_id]);
    }
  });
};
